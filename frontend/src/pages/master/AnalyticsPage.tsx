...
export function AnalyticsPage() {
  ...
  if (isLoading) {
    return <div className="page"><SkeletonList count={4} /></div>;
  }
    ...
    <div className="page animate-fade-in">
      ...
      <div className={styles.statsGrid}>
        <div className="stat-card">
          <span className="stat-card-label">{intl.formatMessage({ id: 'analytics.totalBookings' })}</span>
          <span className="stat-card-value">{data?.period?.totalBookings ?? 0}</span>
        </div>
        <div className="stat-card">
          <span className="stat-card-label">{intl.formatMessage({ id: 'analytics.completed' })}</span>
        ...
        <div className="stat-card">
          ...
          <span className="stat-card-value">{((data?.period?.revenue ?? 0) / 100).toFixed(0)} ₴</span>
        </div>
        <div className="stat-card">
          <span className="stat-card-label">{intl.formatMessage({ id: 'analytics.newClients' })}</span>
      ...
      {data?.period?.popularServices && data.period.popularServices.length > 0 && (
        <section style={{ marginTop: 24 }}>
          <h2 className={styles.sectionTitle}>{intl.formatMessage({ id: 'analytics.popularServices' })}</h2>
          <Card padding="none">
            {data.period.popularServices.map((service: { name: string; count: number }, i: number) => (
